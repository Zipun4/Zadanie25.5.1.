...
def testing():
    pytest.driver = webdriver.Chrome('E:\WebDriverChrome/chromedriver.exe')
    pytest.driver.implicitly_wait(10)
    pytest.driver.get('http://petfriends.skillfactory.ru/login')
    pytest.driver.maximize_window()
    pytest.driver.find_element(By.ID, 'email').send_keys('fdima894@gmail.com')
    pytest.driver.find_element(By.ID, 'pass').send_keys('12345qw')
    pytest.driver.implicitly_wait(10)
    WebDriverWait(pytest.driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button[type='submit']"))).click()
    WebDriverWait(pytest.driver, 10).until(EC.element_to_be_clickable((By.XPATH, "//a[contains(text(),'Мои питомцы')]"))).click()
...
def test_all_pets_present():
    ...
    NPets = int(pytest.driver.find_element(By.CSS_SELECTOR,'html>body>div>div>div').text.split("\n")[1].split(":")[1].strip())
